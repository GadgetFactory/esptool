...
def _validate_config_file(file_path, verbose=False):
    ...
    try:
        ...
        # Only consider it a valid config file if it contains [pesptool] section
        if cfg.has_section("pesptool"):
            if verbose:
                unknown_opts = list(set(cfg.options("pesptool")) - set(CONFIG_OPTIONS))
                unknown_opts.sort()
...
def _find_config_file(dir_path, verbose=False):
    for candidate in ("pesptool.cfg", "setup.cfg", "tox.ini"):
...
def load_config_file(verbose=False):
    ...
    env_var_path = os.environ.get("pesptool_CFGFILE")
    if env_var_path is not None and _validate_config_file(env_var_path):
        ...
        os_config_dir = (
            f"{home_dir}/.config/pesptool"
            if os.name == "posix"
            else f"{home_dir}/AppData/Local/pesptool/"
        )
    ...
    cfg["pesptool"] = {}  # Create an empty pesptool config for when no file is found

    if cfg_file_path is not None:
        # If config file is found and validated, read and parse it
        cfg.read(cfg_file_path)
        if verbose:
            msg = " (set with pesptool_CFGFILE)" if set_with_env_var else ""
            log.print(
