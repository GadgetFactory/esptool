...
#  - `pytest test_pesptool_sdm.py --chip esp32s2 --port /dev/ttyUSB0 --baud 115200`
#
...
from test_pesptool import pesptoolTestCase, arg_chip, pesptool, pytest
...
class TestSecureDownloadMode(pesptoolTestCase):
    expected_chip_name = pesptool.util.expand_chip_name(arg_chip)

    def test_auto_detect(self):
        output = self.run_pesptool("get-security-info", chip="auto")
    ...
    def test_sdm_incompatible_commands(self):
        output = self.run_pesptool_error("flash-id")  # flash-id
        assert "The 'flash-id' command is not available" in output

        output = self.run_pesptool_error("read-flash 0 10 out.bin")  # read-flash
        assert "The 'read-flash' command is not available" in output

    # Commands supported in SDM
    def test_sdm_compatible_commands(self):
        output = self.run_pesptool("write-flash 0x0 images/one_kb.bin")  # write-flash
        ...
        output = self.run_pesptool("erase-region 0 4096")  # erase-region
        ...
        output = self.run_pesptool("get-security-info")
