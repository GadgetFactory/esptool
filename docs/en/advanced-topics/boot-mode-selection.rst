...
.. only:: esp8266
   ...
   On many development boards with built-in USB/Serial, this is done for you and ``pesptool`` can automatically reset the board into bootloader mode. For other configurations, you will need to follow these steps:
   ...
   | GPIO0 Input   | Mode                                   |
   +===============+========================================+
   | Low/GND       | ROM serial bootloader for pesptool      |
   +---------------+----------------------------------------+
...
.. only:: not esp8266
   ...
   On many development boards with built-in USB/Serial, ``pesptool`` can automatically reset the board into bootloader mode. For other configurations or custom hardware, you will need to check the orientation of some "strapping pins" to get the correct boot mode:
   ...
   .. list-table::
      ...
      * - {IDF_TARGET_STRAP_BOOT_GPIO} Input
        - Mode
      * - Low/GND
        - ROM serial bootloader for pesptool
      * - High/VCC
...
``pesptool`` resets {IDF_TARGET_NAME} automatically by asserting ``DTR`` and ``RTS`` control lines of the USB to serial converter chip, i.e., FTDI, CP210x, or CH340x. The ``DTR`` and ``RTS`` control lines are in turn connected to ``{IDF_TARGET_STRAP_BOOT_GPIO}`` and ``EN`` (``CHIP_PU``) pins of {IDF_TARGET_NAME}, thus changes in the voltage levels of ``DTR`` and ``RTS`` will boot the {IDF_TARGET_NAME} into Firmware Download mode.

.. note::

      When developing ``pesptool``, keep in mind ``DTR`` and ``RTS`` are active low signals, i.e., ``True`` = pin @ 0V, ``False`` = pin @ VCC.
...
-  Some OS and/or drivers may activate ``RTS`` and or ``DTR`` automatically when opening the serial port (true only for some serial terminal programs, not ``pesptool``), pulling them low together and holding the ESP in reset. If ``RTS`` is wired directly to ``EN`` then RTS/CTS "hardware flow control" needs to be disabled in the serial program to avoid this.
...
Make the following connections for ``pesptool`` to automatically enter the bootloader of an {IDF_TARGET_NAME} chip:
...
In general, you should have no problems with the official Espressif development boards. However, ``pesptool`` is not able to reset your hardware automatically in the following cases:
...
.. note::
   ...
   If pesptool is able to reset the chip but for some reason the chip is not entering into bootloader mode then hold down the Boot button (or pull down ``{IDF_TARGET_STRAP_BOOT_GPIO}``) while you start pesptool and keep it down during reset.
...
.. only:: not esp8266
   ...
   -  ``DOWNLOAD_BOOT(UART0/UART1/SDIO_REI_REO_V2)`` or ``DOWNLOAD(USB/UART0)`` - {IDF_TARGET_NAME} is in download flashing mode (suitable for pesptool)
   ...
   .. list::
      ...
      -  ``mode: AAA, clock div: N``. SPI flash access mode. Read from the bootloader header, correspond to the ``--flash-mode`` and ``--flash-freq`` arguments supplied to ``pesptool write-flash`` or ``pesptool elf2image``.
   ...
   You can compare these values to the software bootloader image by running ``pesptool --chip {IDF_TARGET_PATH_NAME} image-info /path/to/bootloader.bin`` to dump image info including a summary of each segment. Corresponding details will also be found in the bootloader ELF file headers.
